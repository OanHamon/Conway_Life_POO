...
vector<vector<int>> FileManager::getGrid() {
    ...
    if (read(&data)) {
        ...
        if (rows <= 0 || cols <= 0) {
            cerr << "Erreur: dimensions n�gatives ou nulles." << endl;
            return {};
        }
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (!(ss >> grid[i][j])) {
                    cerr << "Erreur: donn�es insuffisantes ou invalides � la position (" << i << "," << j << ")." << endl;
                    return {};
                }
            }
        }
...
bool FileManager::saveGrid(const vector<vector<int>>& _grid) {
    ...
    for (const auto& r : _grid) {
        if ((int)r.size() != col) {
            cerr << "Erreur: lignes de tailles diff�rentes." << endl;
            return false;
        }
    }
